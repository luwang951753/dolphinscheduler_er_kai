...
public class LoginHandlerInterceptor implements HandlerInterceptor {
    ...
    private static final String DOLPHINSCHEDULER_UI_PATH = "/dolphinscheduler/ui";
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        if (isUiResourceRequest(request)) {
            return true;
        }
    ...
    private boolean isUiResourceRequest(HttpServletRequest request) {
        if (isUiPath(stripContextPath(request.getRequestURI(), request.getContextPath()))) {
            return true;
        }
        String servletPath = request.getServletPath();
        if (StringUtils.isNotEmpty(servletPath) && isUiPath(servletPath)) {
            return true;
        }
        String pathInfo = request.getPathInfo();
        return StringUtils.isNotEmpty(pathInfo) && isUiPath(pathInfo);
    }
    ...
    private boolean isUiPath(String path) {
        ...
        return ROOT_PATH.equals(path)
                || UI_PATH.equals(path)
                || path.startsWith(UI_PATH + ROOT_PATH)
                || DOLPHINSCHEDULER_UI_PATH.equals(path)
                || path.startsWith(DOLPHINSCHEDULER_UI_PATH + ROOT_PATH);
    }

    private String stripContextPath(String requestUri, String contextPath) {
        if (StringUtils.isEmpty(requestUri) || StringUtils.isEmpty(contextPath)) {
            return requestUri;
        }
    ...
    private String normalizePath(String path) {
        if (StringUtils.isEmpty(path) || path.startsWith(ROOT_PATH)) {
            return path;
        }
