...
implements SinkWriter<SeaTunnelRow, DorisCommitInfo, DorisSinkState> {
    ...
    public String getAvailableBackend() {
        ...
        while (this.pos < tmp) {
            ...
            if (!this.tryHttpConnection(res)) {
                String localMappedBackend = this.localDockerBackendFallback(res);
                if (localMappedBackend == null || !this.tryHttpConnection(localMappedBackend)) continue;
                log.warn("Use local Docker mapped Doris backend {} instead of FE reported backend {}", (Object)localMappedBackend, (Object)res);
                return localMappedBackend;
            }
            ++this.pos;
            return res;
        }
    ...
    private String localDockerBackendFallback(String backend) {
        if (backend == null) {
            return null;
        }
        int portStart = backend.lastIndexOf(58);
        if (portStart <= 0 || portStart >= backend.length() - 1) {
            return null;
        }
        String host = backend.substring(0, portStart);
        String port = backend.substring(portStart + 1);
        if (!this.isDockerPrivateAddress(host)) {
            return null;
        }
        return "127.0.0.1:" + port;
    }

    private boolean isDockerPrivateAddress(String host) {
        if (host == null) {
            return false;
        }
